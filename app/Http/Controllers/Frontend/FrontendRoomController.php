...
class FrontendRoomController extends Controller
{
    ...
    public function BookingSearch(Request $request){
         ...
         $check_date_booking_ids=RoomBookingDate::whereIn('book_date',$dt_array)
         ->distinct()->pluk('booking_id')->toArray();
         $rooms=Room::withCount('room_numbers')->where('status',1)
         ->get();
         return view('frontend.room.search_room',
         compact('rooms','check_data_booking_ids'));
 ...
 public function SearchRoomDetails(Request $request ,$id){
    ...
    $otherRooms=Room::where('id', '!=',$id)->orderBy('id','DESC')->limit(2)->get();
    $room_id=$id;
    return view('frontend.room.search_room_details',compact('roomdetails',
    'multiImage','facility','otherRooms','room_id'
 ));
