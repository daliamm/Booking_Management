...
class RoomListController extends Controller
{
   public function ViewRoomList(){
     ...
     $room_number_list=RoomNumber::with(
        ['room_type','last_booking.booking:id,check_in,check_out,status,code,name,phone'])
     ...
     ->select('room_numbers.*','room_numbers.id as id','room_types.name','bookings.id as booking_id',
     'bookings.check_in','bookings.check_out','bookings.name as customer_name',
     'bookings.phone as customer_phone','bookings.status as booking_status','bookings.code as booking_no'
    )
...
public function StoreRoomList(Request $request)
    {
        ...
        if($request->available_room < $request->number_of_rooms){
         ...
         if($room->room_capacity < $request->number_of_person){
            $notification = array(
                'message' => 'You Enter Maximum Number of Gusest !',
                'alert-type' => 'error'
            );
            return redirect()->back()->with($notification);
         }
        ...
        $subtotal = $room->price * $days * $request->number_of_rooms;
